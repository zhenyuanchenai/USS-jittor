...
class Kmeans(object):
    def __init__(self, nmb_clusters, nredo=20):
        ...
        nredo (int): The number os trials in clustering.  #聚类次数
        """
    ...
    def cluster(self, npdata1, npdata2=None, use_pca=False, save_centroids=False):
        ...
        # Clustering the data
        labels, loss, centroids = self.run_kmeans(  #聚类结果、损失、和中心
            npdata1=npdata1,
            npdata2=npdata2,
    ...
    def preprocess_features(self, npdata1, npdata2, pca_dim=128, use_pca=False):
        ...
        npdata1 = npdata1 / (row_sums[:, np.newaxis] + 1e-8)    #特征做一下归一化
        if npdata2 is not None:
            row2_sums = np.linalg.norm(npdata2, axis=1)         #特征做一下归一化
