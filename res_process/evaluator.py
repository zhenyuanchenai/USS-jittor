...
def get_dataset(args, mode, threshold=None, match=None):
    ...
    dataset = EvalDataset(predict_path, gt_path, threshold=threshold, match=match, infer_sam=False)
    dataset.set_attrs(
        batch_size=1,
        shuffle=False,
        num_workers=0,
...
def evaluator(args, mode, num_classes, thresholds, matches):
    ...
    for t in thresholds:
        ...
        FMeasure = FMeasure.item() / len(loader) * 100
        ACC = ACC.item() * 100 / len(loader)
...
def evaludation(args, mode):
    ...
    if args.curve:
        thresholds = [
            threshold / 100.0 for threshold in range(args.min, args.max + 1)      #20-80的阈值
        ]
    else:
        thresholds = [args.t / 100.0]      #阈值是0
    matches = match(args, mode, args.num_classes, threshold=thresholds)
    log = evaluator(
        args, mode,
        num_classes=args.num_classes,          #不同阈值下的结果
        thresholds=thresholds,
        matches=matches,
    )
    return log
