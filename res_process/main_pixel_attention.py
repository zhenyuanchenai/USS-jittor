...
def main():
    ...
    for name, param in model.named_parameters():                  #只微调像素的投影head和原型
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.queue_length > 0 and epoch >= args.epoch_queue_starts and queue is None:
            queue = jt.zeros(
                (
                    len(args.crops_for_assign),                    #搞一个队列
                    args.queue_length // jt.world_size,            #3840//卡数
                    args.feat_dim
                )
            )
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue):
    ...
    for it, inputs in enumerate(train_loader):
        ...
        with jt.no_grad():                                             #聚类weight归一化
            model.prototypes.weight.assign(
                model.prototypes.weight.normalize(dim=1, p=2))

        # ============ multi-res forward passes ... ============
        embedding, output = model(inputs,threshold=int(args.threshold))
        ...
        for i, crop_id in enumerate(args.crops_for_assign):
            ...
            for v in np.delete(np.arange(np.sum(args.nmb_crops)), crop_id):
                ...
                subloss -= jt.mean(jt.sum(q * jt.nn.log_softmax(x, dim=1), dim=1))          #对像素的global结果做聚类的相互监督，跟队列
        ...
        for name, param in model.named_parameters():
            if "prototypes" in name:
                if iteration >= args.freeze_prototypes_niters:                              #从0 iter就开始训练prototype
                    param.start_grad()
