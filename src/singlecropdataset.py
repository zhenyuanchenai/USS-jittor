...
class EvalDataset(datasets.ImageFolder):
    ...
    def __init__(self, prediction_root, target_root, match=None, threshold=None, infer_sam=True):
        ...
        for path, _ in self.imgs:
            ...
            if infer_sam:
                self.logit_lst.append(os.path.join(prediction_root, categroy, name[:-4] + "lopgits_all.npy"))
            else:
...
class InferImageFolder(datasets.ImageFolder):
    ...
    def __init__(self, root, transform, image_label_root=None, transform_raw=None,num_gpus=1):
        ...
        self.image_label_root = image_label_root
        if image_label_root is not None:
            with open(image_label_root,"r") as f:
                lines = f.readlines()
    ...
    def __getitem__(self, index):
        ...
        height, width = img.size[1], img.size[0]  #img.size会将原来image的H，W反过来，变成W，H。所以需要调换索引。
        ...
        if self.image_label_root is not None:
            #image_path = self.image_path[index]
            image_label = self.image_label[index]
        
        if self.image_label_root is not None and self.transform_raw is not None:
            return img_raw, img, path, height, width, image_label
        
        elif self.transform_raw is not None:
            return img_raw, img, path, height, width,
        
        elif self.image_label_root is not None: 
            return img, path, height, width, image_label
...
class PseudoLabelDataset(datasets.ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        pseudo = Image.fromarray(pseudo.permute(1, 2, 0).data.astype(np.uint8))
