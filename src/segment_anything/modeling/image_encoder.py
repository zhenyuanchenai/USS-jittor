...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if use_abs_pos:
            ...
            self.pos_embed = jt.Var(jt.zeros((1, img_size // patch_size, img_size // patch_size, embed_dim)))
...
class Attention(nn.Module):
    ...
    def execute(self, x: jt.Var) -> jt.Var:
        ...
        attn = (q * self.scale).matmul(k.transpose(-2, -1))

        if self.use_rel_pos:
            attn = add_decomposed_rel_pos(attn, q, self.rel_pos_h, self.rel_pos_w, (H, W), (H, W))

        attn = attn.softmax(dim=-1)
        x = (attn.matmul(v)).view(B, self.num_heads, H, W, -1).permute(0, 2, 3, 1, 4).reshape(B, H, W, -1)
        x = self.proj(x)

        return x

def window_partition(x: jt.Var, window_size: int):
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: jt.Var) -> jt.Var:
    ...
    return rel_pos_resized[relative_coords.cast(jt.int32)]
...
def add_decomposed_rel_pos(
    ...
    rel_h = jt.linalg.einsum("bhwc,hkc->bhwk",r_q, Rh)
    rel_w = jt.linalg.einsum("bhwc,wkc->bhwk",r_q, Rw)
