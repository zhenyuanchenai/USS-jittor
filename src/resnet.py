...
class ResNet(nn.Module):
    def __init__(self,
        ...
        if train_mode == 'pixelattn':
            ...
            self.last_layer = nn.Conv2d(num_out_filters * block.expansion,  #投影到类别，前景+背景
                                        num_classes + 1, 1, 1)
    ...
    def execute(self, inputs, gridq=None, gridk=None, mode='train', threshold=None):
        if mode == 'cluster':
            output = self.inference_cluster(inputs, threshold=threshold if threshold is not None else 0.)    #对输入进行聚类
            return output
        ...
        for end_idx in idx_crops:
            ...
            if start_idx == 0:
                if self.train_mode == 'pixelattn':
                    _out = self.execute_pixel_attention(_out, threshold=threshold if threshold is not None else 0.)        #最后一层，前景特征的avg-pool的global结果
                ...
                if self.train_mode == 'pixelattn':
                    _out = self.execute_pixel_attention(_out, threshold=threshold if threshold is not None else 0.)                      #local部分的前景特征的global结果。
                elif self.train_mode == 'pretrain':
